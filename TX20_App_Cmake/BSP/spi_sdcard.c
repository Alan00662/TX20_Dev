...
static uint8_t sdnand_spi_init(sdnand_spi_speed_t speed)
{
    ...
    if (speed >= SDNAND_SPI_SPEED_DUMMY) {
      debug_tx4(" sdnand_spi_init Err1\n");
        return 1;
    }
    ...
    if (HAL_SPI_DeInit(&spi_handle) != HAL_OK) {
      debug_tx4(" sdnand_spi_init Err2\n");
        return 1;
    }
    ...
    if (HAL_SPI_Init(&spi_handle) != HAL_OK) {
      debug_tx4(" sdnand_spi_init Err3\n");
        return 1;
    }
...
void HAL_SPI_MspInit(SPI_HandleTypeDef* hspi)
{
    ...
    if (hspi->Instance == SDNAND_SPI)
    {
        ...
        rcc_periph_clk_init_struct.Xspi1ClockSelection = RCC_SPI45CLKSOURCE_PCLK2;
...
uint8_t spi_sdcard_init(void)
{
    ...
    {
      sdnand_chip_select_disable();
       debug_tx4("sdnand_send_cmd SDNAND_CMD0 Err \n");
        return 1;
    }
    ...
    {
      sdnand_chip_select_disable();
      debug_tx4("sdnand_send_cmd SDNAND_CMD55 Err \n");
        return 1;
    }
    ...
    {
      sdnand_chip_select_disable();
      debug_tx4("sdnand_send_cmd SDNAND_CMD16 Err \n");
        return 1;
    }
    if (sdnand_recv_data(sdnand_info.csd, sizeof(sdnand_info.csd)) != 0)
    {
      sdnand_chip_select_disable();
      debug_tx4("sdnand_recv_data sdnand_info.csd Err \n");
        return 1;
    }
    ...
    {
      sdnand_chip_select_disable();
      debug_tx4("sdnand_send_cmd SDNAND_CMD9 Err \n");
        return 1;
    }
    if (sdnand_recv_data(sdnand_info.csd, sizeof(sdnand_info.csd)) != 0)
    {
      sdnand_chip_select_disable();
      debug_tx4("sdnand_recv_data sdnand_info.csd 2 Err \n");
        return 1;
    }
    sdnand_chip_select_disable();
...
{
    debug_tx4("SD CARD Chip Size: %dMB\r\n", sdnand_info.chip_size >> 20);
    debug_tx4("SD CARD Block Size: %dB\r\n", sdnand_info.block_size);
    debug_tx4("SD CARD Block Number: %d\r\n", sdnand_info.block_num);
    debug_tx4("SD CARD Logic Block Size: %dB\r\n", sdnand_info.logic_block_size);
    debug_tx4("SD CARD Logic Block Number: %d\r\n", sdnand_info.logic_block_num);
